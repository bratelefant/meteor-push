...
class PushHandle extends EventState {
  ...
  unregister(successHandler, errorHandler) {
    if (this.push) {
      ...
      errorHandler(
        new Error('PushHandle.unregister, Error: "Push not configured"')
      );
    }
  }
      ...
      Meteor.startup(() => {
        if (typeof PushNotification !== "undefined") {
          ...
          this.push.on("registration", (data) => {
            ...
            const oldToken =
              (storedToken && storedToken.apn) ||
              (storedToken && storedToken.android) ||
              null;
            this.log(
              "device registration has been triggered with this data: ",
              data
            );
            if (
              data &&
              data.registrationId &&
              oldToken !== data.registrationId
            ) {
          ...
          this.push.on("notification", (data) => {
            ...
            if (data.additionalData.ejson) {
              if (
                ...
                try {
                  ...
                  this.log(
                    "PushHandle.Parsed.EJSON.Payload.Error",
                    err.message,
                    data.payload
                  );
                }
            ...
            if (data.additionalData.foreground) {
              this.log(
                "PushHandle.Message: Got message while app is open:",
                data
              );
              // TODO handle this
              ...
              this.log(
                "PushHandle.Startup: Got message while app was closed/in background:",
                data
              );
                ...
                if (Meteor.isCordova) {
                  if (removed) {
                    ...
                    deviceStorage.setItem(
                      "token",
                      doc.token.apn || doc.token.android
                    );
                  }
                }
              }

...
await Push.notifications.createIndexAsync({ createdAt: 1 });
await Push.notifications.createIndexAsync({ sent: 1 });
await Push.notifications.createIndexAsync({ sending: 1 });
await Push.notifications.createIndexAsync({ delayUntil: 1 });
await Push.notifications.createIndexAsync({ userId: 1 });
...
const validateDocument = (notification) => {
  ...
  check(notification, {
    ...
    delayUntil: Match.Optional(
      Match.Where((timestamp) => new Date(timestamp).getTime() > 0)
    ), // like Date.now()
...
Push.send = async (content) => {
  const currentUser =
    (Meteor.isClient && Meteor.userId()) ||
    (Meteor.isServer && (content.createdBy || "<SERVER>")) ||
    null;
  ...
  const result = await Push.notifications.insertAsync(notification);
  return result;
};

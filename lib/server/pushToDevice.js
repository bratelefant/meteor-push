...
Push.Configure = (serverConfig) => {
  ...
  const fcm = admin.initializeApp({
    credential: admin.credential.cert(
      serverConfig.firebaseAdmin &&
        serverConfig.firebaseAdmin.serviceAccountData
    ),
    databaseURL:
      serverConfig.firebaseAdmin && serverConfig.firebaseAdmin.databaseURL,
  });
  ...
  if (serverConfig.firebaseAdmin) {
    ...
    if (!serverConfig.firebaseAdmin.serviceAccountData) {
      console.error(
        "ERROR: Push server could not find Android serviceAccountData information"
      );
    }
    if (!serverConfig.firebaseAdmin.databaseURL) {
      console.error(
        "ERROR: Push server could not find databaseURL information"
      );
    }
    self.sendNotification = (userTokens, mongoNote) =>
      sendNotification(
        isDebug,
        fcmConnections,
  ...
  const querySend = (query, mongoNote) => {
    ...
    Push.appCollection.find(query).map((app) => {
      ...
      self.sendNotification(
        app.token.apn || app.token.android || app.token.web,
        mongoNote
      );
    });
    ...
    if (isDebug) {
      ...
      if (!countApn.length && !countAndroid.length && !countWeb.length) {
        if (!Push.appCollection.findOne()) {
          console.log(
            'Push, GUIDE: The "Push.appCollection" might be empty. No clients have registered on the server yet...'
          );
        }
        ...
        if (
          !Push.appCollection.findOne({ "token.android": { $exists: true } })
        ) {
          console.log(
            'Push, GUIDE: The "Push.appCollection" - No ANDROID clients have registered on the server yet...'
          );
        }
      } else if (!countWeb.length) {
        if (!Push.appCollection.findOne({ "token.web": { $exists: true } })) {
          console.log(
            'Push, GUIDE: The "Push.appCollection" - No Web clients have registered on the server yet...'
          );
        }
      }
    }
  ...
  self.serverSend = (mongoNote) => {
    ...
    if (mongoNote.token || mongoNote.tokens) {
      ...
      if (isDebug) {
        console.log(
          'Push: Send message "' + mongoNote.title + '" via token(s)',
          tokenList
        );
      }
      query = {
      ...
      const tokenIdsList = mongoNote.tokenId
        ? [mongoNote.tokenId]
        : mongoNote.tokenIds;
      if (isDebug) {
        console.log(
          'Push: Send message "' + mongoNote.title + '" via token Id(s)',
          tokenIdsList
        );
      }
      query = {
      ...
      const userIdsList = mongoNote.userId
        ? [mongoNote.userId]
        : mongoNote.userIds;
      if (isDebug) {
        console.log(
          'Push: Send message "' + mongoNote.title + '" to user: ',
          userIdsList
        );
      }
      query = {
    ...
    if (query) {
      ...
      if (isDebug) {
        throw new Error(
          "Push.send: please set option token/tokens, tokenId/tokenIds, userId/userIds"
        );
      }
    }
  };
  ...
  if (serverConfig.defaults && serverConfig.defaults.sendInterval !== null) {
    const sendNotification = (mongoNote) => {
      ...
      const now = Date.now();
      const timeoutAt =
        now + serverConfig.defaults && serverConfig.defaults.sendTimeout;
      ...
      if (reserved) {
        ...
        if (
          !(serverConfig.defaults && serverConfig.defaults.keepNotifications)
        ) {
    ...
    sendWorker(
      () => {
        ...
        try {
          ...
          const batchSize =
            (serverConfig.defaults && serverConfig.defaults.sendBatchSize) || 1;
          const now = Date.now();
          ...
          pendingNotifications.map((mongoNote) => {
            try {
              ...
              if (isDebug) {
                ...
                console.log(
                  'Push: Could not send notification id: "' +
                    mongoNote._id +
                    '", Error: ' +
                    error.message
                );
